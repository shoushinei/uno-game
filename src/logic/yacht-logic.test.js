...
describe('scoreAll — 役計算', () => {
  ...
  it('チョイス = 合計、1〜6 = その目だけの合計', () => {
    const d = [1, 1, 3, 6, 6];
    expect(score(d, 'choice')).toBe(17);
...
describe('bestHand — 最高役', () => {
  ...
  it('ビッグストレートは合計30のチョイスと同点でも役名が優先される', () => {
    // 2+3+4+5+6=20 なのでチョイス20 < 30。素直にB-straight
    expect(bestHand([2, 3, 4, 5, 6])).toEqual({ category: 'big-straight', score: 30 });
  });

  it('バラバラの手はチョイス（合計）が最高', () => {
    const b = bestHand([1, 1, 2, 3, 5]);
    expect(b.score).toBe(12);
    expect(b.category).toBe('choice');
  });

  it('4個同一はフォーナンバーズ（チョイスと同点なら格上の役名）', () => {
