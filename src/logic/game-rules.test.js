...
describe('applyTrumpSkip', () => {
  it('phase が uno になる', () => {
    const g = makeGame({ phase: 'trump', trumpHands: { p1: [] }, unoHands: { p1: ['red-1'] } });
    applyTrumpSkip(g, 'p1', 'Alice');
    expect(g.phase).toBe('uno');
  });

  it('logMsg にプレイヤー名が含まれる', () => {
    const g = makeGame({ phase: 'trump', trumpHands: { p2: [] }, unoHands: { p2: ['red-1'] } });
...
describe('applyTrumpSkip — 追加（正常系）', () => {
  ...
  it('何度呼んでも phase は uno のまま（冪等性）', () => {
    const g = makeGame({ trumpHands: { p1: [] }, unoHands: { p1: ['red-1'] } });
    applyTrumpSkip(g, 'p1', 'Alice');
    applyTrumpSkip(g, 'p1', 'Alice');
    expect(g.phase).toBe('uno');
  });
...
describe('applyUnoSkip', () => {
  it('次のプレイヤーのトランプターンへ進める', () => {
    const g = makeGame({ phase: 'uno', ci: 0, trumpHands: { p1: ['♠2'] }, unoHands: { p1: [] } });
  ...
  it('自分が親の場合、色変更権限を行使せずに hasParent をクリアする', () => {
    const g = makeGame({ phase: 'uno', ci: 0, hasParent: 'p1', trumpHands: { p1: ['♠2'] }, unoHands: { p1: [] } });
  ...
  it('自分が親でない場合は hasParent に影響しない', () => {
    const g = makeGame({ phase: 'uno', ci: 0, hasParent: 'p2', trumpHands: { p1: ['♠2'] }, unoHands: { p1: [] } });
    applyUnoSkip(g, 'p1', 'Alice');
  ...
  it('反時計回り（dir=-1）でも正しく前のプレイヤーへ進む', () => {
    const g = makeGame({ phase: 'uno', ci: 0, dir: -1, trumpHands: { p1: ['♠2'] }, unoHands: { p1: [] } });
    applyUnoSkip(g, 'p1', 'Alice');
...
// ========================================
// ★バグ4 回帰テスト★
// 「親になって、親の権限（色変更）を行使したタイミングで、実は
//  トランプ・UNOの両方がすでに0枚（＝本来は上がっているはず）だった」
// というケースで finalizeIfBothHandsEmpty が一度も呼ばれず、
// rankings にも order 除外にも反映されない不具合の回帰テスト。
//
// これが「親になって権限を使わなかった状態で上がった時だけ順位が
// 決まらない」というユーザー報告の再現条件だった。
// ========================================
describe('applyParentColorChange — バグ4回帰（権限行使タイミングでの上がり判定漏れ）', () => {
  it('★重要★ 色変更の時点でトランプ・UNOともに0枚なら、その場で上がり確定（rankings追加・order除外）されること', () => {
    const g = {
      order: ['p1', 'p2', 'p3'],
      ci: 0, dir: 1, phase: 'uno',
      hasParent: 'p1',
      unoCurrentColor: 'red',
      rankings: [],
      trumpHands: { p1: [], p2: ['S3'], p3: ['H4'] }, // p1のトランプは既に0枚
      unoHands:   { p1: [], p2: ['red-1'], p3: ['blue-2'] }, // p1のUNOも既に0枚
    };

    const result = applyParentColorChange(g, 'p1', 'red', 'Alice');

    expect(result).not.toBeNull();
    expect(result.finished).toBe(true);
    expect(g.rankings[0].id).toBe('p1'); // 上がり順として記録される
    expect(g.order).not.toContain('p1'); // 手番から除外される
  });

  it('色変更の時点でUNOのみ0枚・トランプはまだ残っている場合は、従来通り上がらず次のトランプフェイズへ進む', () => {
    const g = {
      order: ['p1', 'p2', 'p3'],
      ci: 0, dir: 1, phase: 'uno',
      hasParent: 'p1',
      unoCurrentColor: 'red',
      rankings: [],
      trumpHands: { p1: ['S9'], p2: ['S3'], p3: ['H4'] }, // p1のトランプはまだ残っている
      unoHands:   { p1: [], p2: ['red-1'], p3: ['blue-2'] }, // p1のUNOのみ0枚
    };

    const result = applyParentColorChange(g, 'p1', 'blue', 'Alice');

    expect(g.rankings.length).toBe(0); // 上がり扱いにはならない
    expect(g.order).toContain('p1'); // まだゲームに残る
    expect(g.phase).toBe('trump'); // 次はトランプフェイズへ
  });

  it('色変更の時点でトランプ・UNOともにまだ残っている場合は、これまで通り自分のUNOフェイズ行動に続く（手番は進まない）', () => {
    const g = {
      order: ['p1', 'p2', 'p3'],
      ci: 0, dir: 1, phase: 'uno',
      hasParent: 'p1',
      unoCurrentColor: 'red',
      rankings: [],
      trumpHands: { p1: ['S9'], p2: ['S3'], p3: ['H4'] },
      unoHands:   { p1: ['red-5'], p2: ['red-1'], p3: ['blue-2'] },
    };

    const result = applyParentColorChange(g, 'p1', 'green', 'Alice');

    expect(g.rankings.length).toBe(0);
    expect(g.hasParent).toBeNull(); // 権限自体は使用済みになる
    expect(result.turnAdvanced).toBeFalsy();
  });
});